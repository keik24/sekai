...
const VARIANT_META: Record<string, {
  ...
  japan: {
    title: 'Sekai - リアルタイム日本&世界ニュースダッシュボード',
    description: 'リアルタイムで日本全国のニュース、災害情報、経済データを一元監視。世界のニュースも同時追跡。地震速報、気象警報、地方ニュースを地図と連動して表示。',
    keywords: 'Sekai, japan news, 日本ニュース, 世界ニュース, 地震速報, 災害情報, 気象警報, 日本経済, 株式市場, 日経平均, 防災, NHK, 朝日新聞, 読売新聞, リアルタイムニュース, 地方ニュース',
    url: 'https://sekai.net/',
    siteName: 'Sekai',
    shortName: 'Sekai',
...
function htmlVariantPlugin(): Plugin {
  return {
    ...
    transformIndexHtml(html) {
      ...
      // Inject GA4 if VITE_GA4_ID or GA4_ID is set
      const ga4Id = process.env.VITE_GA4_ID || process.env.GA4_ID;
      console.log('[vite] GA4 ID:', ga4Id ? `${ga4Id.slice(0, 4)}...` : 'not set');
      if (ga4Id) {
        const ga4Script = `
    <!-- Google Analytics 4 -->
